...
def dev_network_d(input_shape):
    ...
    intermediate = Dense(1000,
                         activation='relu',
                         kernel_regularizer=regularizers.l2(0.01),
                         name='hl1')(x_input)
    intermediate = Dense(250,
                         activation='relu',
                         kernel_regularizer=regularizers.l2(0.01),
                         name='hl2')(intermediate)
    intermediate = Dense(20,
                         activation='relu',
                         kernel_regularizer=regularizers.l2(0.01),
...
def deviation_loss(y_true, y_pred):
    ...
    ref = K.random_normal(shape=(5000, ), mean=0, stddev=1, dtype='float32')
...
def batch_generator_sup(x, outlier_indices, inlier_indices, batch_size,
                        nb_batch, rng):
    """batch generator
    """
...
def input_batch_generation_sup(x_train, outlier_indices, inlier_indices,
                               batch_size, rng):
    '''
    ...
    for i in range(batch_size):
        if (i % 2 == 0):
            ...
            training_labels += [1]
    return np.array(ref), np.array(training_labels, dtype="float32")
...
def input_batch_generation_sup_sparse(x_train, outlier_indices, inlier_indices,
    ...
    return ref, np.array(training_labels, dtype="float32")


def load_model_weight_predict(model_name,
                              input_shape,
                              network_depth,
                              x_test,
                              run=0):
...
def run_devnet(args):
    ...
    for nm in names:
        ...
        for i in np.arange(runs):
            ...
            n_outliers = len(outlier_indices)
            print("Training data size: %d, No. outliers: %d, No. n_noise: %d" \
                 % (x_train.shape[0], n_outliers, n_noise))
            ...
            model_name = "./model/" + str(
                i) + "_devnet_" + filename + "_" + str(
                    args.cont_rate
                ) + "cr_" + str(args.batch_size) + "bs_" + str(
            ...
            log_dir = "./logs/{0:%Y-%m-%dT%H-%M-%S/}".format(datetime.now())
            tensorboard_callback = TensorBoard(log_dir, histogram_freq=1)

            model.fit(batch_generator_sup(x_train, outlier_indices,
                                          inlier_indices, batch_size, nb_batch,
                                          rng),
            ...
            scores = load_model_weight_predict(model_name, input_shape,
                                               network_depth, x_test, i)
            test_time += time.time() - start_time
            np.savetxt('scores.txt', scores, '%.6f')
            np.savetxt('y_test.txt', y_test, '%.6f')
...
parser.add_argument("--network_depth",
                    choices=['1', '2', '4'],
                    default='2',
...
parser.add_argument("--data_set",
                    type=str,
                    default='annthyroid_21feat_normalised',
...
parser.add_argument(
    ...
    type=str,
    default=
    './results/devnet_auc_performance_30outliers_0.02contrate_2depth_10runs.csv',
    help="the output file path")
parser.add_argument("--ramdn_seed",
                    type=int,
                    default=42,
